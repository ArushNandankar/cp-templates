...
/*
 * Given an array a that contains n elements and the operation op that
 * satisfies associative property: (x op y) op z=x op (y op z) is true for
 * any x, y, z.
 *
 * The following implementation of Sqrt Tree can perform the following
 * operations: build in O(nloglogn), answer queries in O(1) and update an
 * element in O(sqrt(n)).
 */
...
{
	return a + b;
	//just change this operation for different
		problems, no change is required inside the code
		}
